    ...
    def perform_create(self, serializer):
        with transaction.atomic():
            ...
            OTP.objects.update_or_create(
                user=user,
                defaults={'email': user.email, 'otp': otp_code}
            )
        send_normal_email({
            'email_body'   : f'کد تأیید شما: {otp_code}',
            'email_subject': 'تأیید ایمیل',
            'to_email'     : user.email,
        })
...
class ResendOTPView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_verified:
            return Response({'detail': 'حساب قبلاً تأیید شده.'}, status=400)
        ...
        send_normal_email({
            'email_body'   : f'کد تأیید جدید: {otp_code}',
            'email_subject': 'ارسال مجدد کد تأیید',
            'to_email'     : user.email,
        })
...
class LogoutView(APIView):
    ...
    def post(self, request):
        s = LogoutUserSerializer(data=request.data)
        s.is_valid(raise_exception=True)
        s.save()
...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        s.is_valid(raise_exception=True)
        s.save()
        return Response({'detail': 'لینک بازنشانی ارسال شد.'})
...
class SetNewPasswordView(APIView):
    ...
    def post(self, request):
        s = SetNewPasswordSerializer(data=request.data)
        s.is_valid(raise_exception=True)
...
class PublicProfileView(APIView):
    """پروفایل عمومی هر کاربر — مثل اینستاگرام"""
    permission_classes = [AllowAny]
    ...
    def get(self, request, username):
        try:
            ...
            return Response({'detail': 'کاربر یافت نشد.'}, status=404)
        s = PublicProfileSerializer(user.profile, context={'request': request})
...
class UserSearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        ).exclude(pk=self.request.user.pk).select_related('profile').order_by('username')[:20]
